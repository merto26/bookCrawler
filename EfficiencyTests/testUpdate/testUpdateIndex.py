...
def testeUpdateIndex(num_repeats):
    """
    ...
    for config in mongo_configs:
        time_taken_median = run_mongo_query(config['host'], config['port'], config['database'], config['collection'], config['query'], update, num_repeats)
        if time_taken_median:
    ...
    graphCreator.createStackedBarGraph(x=["index", "no index"], y1=mysql_times, y2=mongo_times)