...
def createMockData(size, output):
    """
    Erstellt HTML-Datein mit jeweils 5000 generierten Büchern als Testdaten.
    ...
    while bookcounter < size+1:
        ...
        <li class="book">
            ...
            <div class="book-price">Price: {round(price + tax, 2)}</div>
            <div class="book-tax">Price (without tax): {price}</div>
            <div class="book-availability">Availability: {random.randint(0, 100)} in stock</div>
        ...
        print(f"Buch {bookcounter} wurde grade gemocked!")

        if bookcounter%5000 == 0:
            final_html = html_template.format(books=books_html)
            output_html_file = f'C:\\Users\\mert-\\PycharmProjects\\bookCrawlerProject\\HTML Formulare\\5Mioin5k\\{output}{pageCount}.html'
            with open(output_html_file, 'w', encoding="utf-8") as file:
                file.write(final_html)
            print(f"HTML {pageCount} wurde generiert")
...
if __name__ == '__main__':
    ...
    createMockData(5000000, f"MockDataPage")
    endTime = time.time()
